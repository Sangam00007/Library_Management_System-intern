...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Authors</h1>
            <p class="text-sm text-slate-500 mt-1">Manage book authors.</p>
        </div>
        <a href="{{ route('admin.authors.create') }}" class="px-4 py-2 bg-amber-500 hover:bg-amber-600 text-sm font-medium text-white rounded-lg shadow-lg shadow-amber-500/30 transition-all flex items-center gap-2">
    ...
    <div class="bg-white backdrop-blur-sm border border-slate-200 rounded-2xl p-4">
        <form method="GET" action="{{ route('admin.authors.index') }}" class="flex gap-3">
            ...
            @if(request('search'))
                <a href="{{ route('admin.authors.index') }}" class="px-4 py-2 bg-white border border-slate-300 hover:bg-slate-100 text-sm font-medium text-slate-600 rounded-lg transition-colors">Clear</a>
            @endif
        </form>
    </div>
    ...
    <div class="bg-white backdrop-blur-sm border border-slate-200 rounded-2xl overflow-hidden">
        @if($authors->isEmpty())
            <div class="px-6 py-16 text-center">
                ...
                <h3 class="text-sm font-medium text-slate-500">No authors found</h3>
                <p class="text-xs text-slate-500 mt-1">Get started by adding your first author.</p>
                <a href="{{ route('admin.authors.create') }}" class="inline-flex items-center gap-2 mt-4 px-4 py-2 bg-amber-500 hover:bg-amber-600 text-sm font-medium text-white rounded-lg transition-colors">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path></svg>
                    Add Author
                </a>
            </div>
                        ...
                        <tr class="even:bg-slate-50 hover:bg-slate-50 transition-colors">
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    <div class="w-8 h-8 rounded bg-gradient-to-br from-green-500 to-teal-600 flex items-center justify-center flex-shrink-0 shadow-lg shadow-green-500/20">
                                        <span class="text-slate-900 font-bold text-xs">{{ substr($author->name, 0, 1) }}</span>
                                    </div>
                                    <p class="font-medium text-slate-900">{{ $author->name }}</p>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="max-w-xs truncate text-slate-500" title="{{ $author->bio }}">
                                    {{ $author->bio ?: '—' }}
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('admin.authors.edit', $author) }}" class="p-2 text-slate-500 hover:text-amber-600 hover:bg-amber-600/10 rounded-lg transition-colors" title="Edit">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                    </a>
                                    <button type="button" @click="$dispatch('open-delete-modal', { action: '{{ route('admin.authors.destroy', $author) }}', name: '{{ addslashes($author->name) }}' })" class="p-2 text-slate-500 hover:text-red-600 hover:bg-red-500/10 rounded-lg transition-colors" title="Delete">

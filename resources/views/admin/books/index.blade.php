...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Books</h1>
            <p class="text-sm text-slate-500 mt-1">Manage your library's book catalog.</p>
        </div>
        <a href="{{ route('admin.books.create') }}" class="px-4 py-2 bg-amber-500 hover:bg-amber-600 text-sm font-medium text-white rounded-lg shadow-lg shadow-amber-500/30 transition-all flex items-center gap-2">
    ...
    <div class="bg-white backdrop-blur-sm border border-slate-200 rounded-2xl p-4">
        <form method="GET" action="{{ route('admin.books.index') }}" class="flex gap-3">
            ...
            @if(request('search'))
                <a href="{{ route('admin.books.index') }}" class="px-4 py-2 bg-white border border-slate-300 hover:bg-slate-100 text-sm font-medium text-slate-600 rounded-lg transition-colors">Clear</a>
            @endif
        </form>
    </div>
    ...
    <div class="bg-white backdrop-blur-sm border border-slate-200 rounded-2xl overflow-hidden">
        @if($books->isEmpty())
            <div class="px-6 py-16 text-center">
                ...
                <h3 class="text-sm font-medium text-slate-500">No books found</h3>
                <p class="text-xs text-slate-500 mt-1">Get started by adding your first book to the catalog.</p>
                <a href="{{ route('admin.books.create') }}" class="inline-flex items-center gap-2 mt-4 px-4 py-2 bg-amber-500 hover:bg-amber-600 text-sm font-medium text-white rounded-lg transition-colors">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path></svg>
                    Add Book
                </a>
            </div>
                        ...
                        <tr class="even:bg-slate-50 hover:bg-slate-50 transition-colors">
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    @if($book->cover_image)
                                        <img src="{{ asset('storage/' . $book->cover_image) }}" alt="{{ $book->title }}" class="w-10 h-14 rounded-lg object-cover shadow-lg border border-slate-300 flex-shrink-0">
                                    @else
                                        <div class="w-10 h-14 bg-gradient-to-br from-gray-700 to-gray-800 rounded-lg shadow flex items-center justify-center flex-shrink-0 border border-slate-300">
                                            <svg class="w-5 h-5 text-slate-500" fill="currentColor" viewBox="0 0 20 20"><path d="M9 4.804A7.968 7.968 0 005.5 4c-1.255 0-2.443.29-3.5.804v10A7.969 7.969 0 015.5 14c1.669 0 3.218.51 4.5 1.385A7.962 7.962 0 0114.5 14c1.255 0 2.443.29 3.5.804v-10A7.968 7.968 0 0014.5 4c-1.255 0-2.443.29-3.5.804V12a1 1 0 11-2 0V4.804z"></path></svg>
                                        </div>
                                    @endif
                                    <div>
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('admin.books.edit', $book) }}" class="p-2 text-slate-500 hover:text-amber-600 hover:bg-amber-600/10 rounded-lg transition-colors" title="Edit">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                    </a>
                                    <button type="button" @click="$dispatch('open-delete-modal', { action: '{{ route('admin.books.destroy', $book) }}', name: '{{ addslashes($book->title) }}' })" class="p-2 text-slate-500 hover:text-red-600 hover:bg-red-500/10 rounded-lg transition-colors" title="Delete">

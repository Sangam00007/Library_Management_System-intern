<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div class="lg:col-span-2 space-y-6">
        <div class="bg-white backdrop-blur-sm border border-slate-200 rounded-2xl p-6 space-y-5">
            <h3 class="text-lg font-semibold text-slate-900">Book Information</h3>
    ...
    <div class="space-y-6">
        <!-- Classification -->
        <div class="bg-white backdrop-blur-sm border border-slate-200 rounded-2xl p-6 space-y-5">
            <h3 class="text-lg font-semibold text-slate-900">Classification</h3>
        ...
        <!-- Cover Image -->
        <div class="bg-white backdrop-blur-sm border border-slate-200 rounded-2xl p-6 space-y-4">
            <h3 class="text-lg font-semibold text-slate-900">Cover Image</h3>

            @if(isset($book) && $book->cover_image)
                <div class="mb-3">
                    <img src="{{ asset('storage/' . $book->cover_image) }}" alt="{{ $book->title }}" class="w-full max-w-[160px] mx-auto rounded-xl shadow-lg border border-slate-300">
                </div>
            @endif

            <div x-data="{ fileName: '' }">
                <label class="flex flex-col items-center justify-center w-full h-32 border-2 border-slate-300 border-dashed rounded-xl cursor-pointer bg-slate-50/30 hover:bg-slate-50 hover:border-blue-500/50 transition-all">
                    <div class="flex flex-col items-center justify-center pt-5 pb-6">
                        ...
                        <p class="text-xs text-slate-500" x-text="fileName || 'Click to upload (JPG, PNG, WebP — max 2MB)'"></p>
                    </div>
...
<div class="flex items-center justify-end gap-3 pt-2">
    <a href="{{ route('admin.books.index') }}" class="px-5 py-2.5 bg-white border border-slate-300 hover:bg-slate-100 text-sm font-medium text-slate-600 rounded-lg transition-colors">Cancel</a>

...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
            ...
            <p class="text-sm text-slate-500 mt-1">Welcome back, {{ auth('admin')->user()->name ?? 'Administrator' }}. Here's what's happening today.</p>
        </div>
        <div class="flex items-center gap-3">
            <button class="px-4 py-2 bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 text-sm font-medium text-white rounded-lg transition-colors flex items-center gap-2">
            ...
            <button class="px-4 py-2 bg-amber-500 hover:bg-amber-600 text-sm font-medium text-white rounded-lg shadow-lg shadow-amber-500/30 transition-all flex items-center gap-2">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <!-- Total Books -->
        <div class="bg-white border border-slate-200 shadow-sm rounded-2xl p-6 relative overflow-hidden group hover:border-amber-500/50 transition-colors">
            ...
            <div class="flex items-center gap-4 relative z-10">
                ...
                <div>
                    ...
                    <h3 class="text-2xl font-bold text-slate-900 mt-1">{{ number_format($totalBooks) }}</h3>
                </div>
            </div>
        ...
        <!-- Active Members -->
        <div class="bg-white border border-slate-200 shadow-sm rounded-2xl p-6 relative overflow-hidden group hover:border-slate-500/50 transition-colors">
            <div class="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
                <svg class="w-16 h-16 text-slate-500" fill="currentColor" viewBox="0 0 20 20"><path d="M13 6a3 3 0 11-6 0 3 3 0 016 0zM18 8a2 2 0 11-4 0 2 2 0 014 0zM14 15a4 4 0 00-8 0v3h8v-3zM6 8a2 2 0 11-4 0 2 2 0 014 0zM16 18v-3a5.972 5.972 0 00-.75-2.906A3.005 3.005 0 0119 15v3h-3zM4.75 12.094A5.973 5.973 0 004 15v3H1v-3a3 3 0 013.75-2.906z"></path></svg>
            </div>
            <div class="flex items-center gap-4 relative z-10">
                ...
                <div>
                    ...
                    <h3 class="text-2xl font-bold text-slate-900 mt-1">{{ number_format($totalMembers) }}</h3>
                </div>
            </div>
            <div class="mt-4 flex items-center text-sm relative z-10">
                @if($memberChange >= 0)
        ...
        <!-- Books Borrowed -->
        <div class="bg-white border border-slate-200 shadow-sm rounded-2xl p-6 relative overflow-hidden group hover:border-amber-500/30 transition-colors">
            <div class="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
            ...
            <div class="flex items-center gap-4 relative z-10">
                ...
                <div>
                    ...
                    <h3 class="text-2xl font-bold text-slate-900 mt-1">{{ number_format($activeBorrowings) }}</h3>
                </div>
            </div>
            <div class="mt-4 flex items-center text-sm relative z-10">
        ...
        <!-- Fines Collected -->
        <div class="bg-white border border-slate-200 shadow-sm rounded-2xl p-6 relative overflow-hidden group hover:border-emerald-500/30 transition-colors">
            <div class="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
    ...
    <div class="bg-red-50 border border-red-200 rounded-2xl p-5 flex items-center gap-4">
        ...
        <div>
            <h3 class="text-sm font-semibold text-red-800">Overdue Alert</h3>
            <p class="text-sm text-red-600 mt-0.5">{{ $overdueCount }} {{ Str::plural('book', $overdueCount) }} currently overdue. Please follow up with the members.</p>
        </div>
    </div>
    @endif
    ...
    <div class="bg-white border border-slate-200 shadow-sm rounded-2xl overflow-hidden">
        <div class="px-6 py-5 border-b border-slate-200 flex items-center justify-between">
            <h2 class="text-lg font-semibold text-slate-900">Recent Borrowings</h2>
        ...
        @if($recentBorrowings->isEmpty())
            <div class="px-6 py-16 text-center">
                <svg class="w-12 h-12 text-slate-300 mx-auto mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4"></path></svg>
                <h3 class="text-sm font-medium text-slate-500">No borrowings yet</h3>
                <p class="text-xs text-slate-500 mt-1">Borrowing records will appear here once members start borrowing books.</p>
            </div>
        @else
                ...
                <table class="w-full text-left text-sm whitespace-nowrap">
                    ...
                    <tbody class="divide-y divide-slate-200 text-slate-600">
                        ...
                        <tr class="even:bg-slate-50 hover:bg-slate-50 transition-colors">
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    <div class="w-8 h-10 bg-slate-100 rounded shadow flex items-center justify-center flex-shrink-0">
                                        <svg class="w-4 h-4 text-slate-500" fill="currentColor" viewBox="0 0 20 20"><path d="M9 4.804A7.968 7.968 0 005.5 4c-1.255 0-2.443.29-3.5.804v10A7.969 7.969 0 015.5 14c1.669 0 3.218.51 4.5 1.385A7.962 7.962 0 0114.5 14c1.255 0 2.443.29 3.5.804v-10A7.968 7.968 0 0014.5 4c-1.255 0-2.443.29-3.5.804V12a1 1 0 11-2 0V4.804z"></path></svg>
                                    </div>
                                    <div>
                                        <p class="font-medium text-slate-900">{{ $borrowing->book?->title ?? 'Deleted Book' }}</p>
                            ...
                            <td class="px-6 py-4 {{ $borrowing->status === 'issued' && $borrowing->due_date->isPast() ? 'text-red-600 font-medium' : '' }}">
